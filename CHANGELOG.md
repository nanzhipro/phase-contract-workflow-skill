...
- Agent instruction layer simplified to a single source of truth: the
  scaffold now generates only `AGENTS.md`, with `CLAUDE.md` as a symlink to
  it; `.github/copilot-instructions.md` is no longer generated (Copilot reads
  `AGENTS.md` natively). `planctl doctor` and `finalize` health checks now
  verify that `AGENTS.md` exists and `CLAUDE.md` is a symlink resolving to
  it — a byte-identical regular file only warns, a diverging one is an
  error. SKILL.md, references, and READMEs updated accordingly.
