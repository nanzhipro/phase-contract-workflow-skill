...
class PlanCtl
  ...
  #   * Agent instruction files: AGENTS.md present; CLAUDE.md is a symlink
  #     to AGENTS.md (a byte-identical regular file is accepted with a warning)
  def doctor
    require 'digest'
    problems = []
    warnings = []
    ...
    agents_md = @repo_root.join('AGENTS.md')
    claude_md = @repo_root.join('CLAUDE.md')
    if !agents_md.file?
      warnings << 'No AGENTS.md found; it is the single source of truth for agent instructions.'
      warnings << 'CLAUDE.md exists but its symlink target AGENTS.md is missing.' if claude_md.symlink?
    elsif claude_md.symlink?
      resolved = begin
        claude_md.realpath
      rescue SystemCallError
        nil
      end
      if resolved == agents_md.realpath
        puts 'Agent instructions: CLAUDE.md -> AGENTS.md symlink ok'
      else
        problems << "CLAUDE.md is a symlink but does not resolve to AGENTS.md (target: #{claude_md.readlink})."
      end
    elsif claude_md.file?
      if Digest::SHA256.hexdigest(claude_md.read) == Digest::SHA256.hexdigest(agents_md.read)
        warnings << 'CLAUDE.md is a byte-identical copy of AGENTS.md; replace it with a symlink: `ln -sf AGENTS.md CLAUDE.md`.'
      else
        problems << 'CLAUDE.md diverges from AGENTS.md; replace it with a symlink: `ln -sf AGENTS.md CLAUDE.md`.'
      end
    else
      warnings << 'CLAUDE.md missing; create it as a symlink to AGENTS.md: `ln -s AGENTS.md CLAUDE.md`.'
    end
  ...
  def build_finalize_health(state)
    ...
    claude_md = @repo_root.join('CLAUDE.md')
    if !agents_md.file?
      notes << 'no AGENTS.md found (single source of truth for agent instructions)'
    elsif claude_md.symlink?
      resolved = begin
        claude_md.realpath
      rescue SystemCallError
      ...
      if resolved == agents_md.realpath
        notes << 'agent instructions: CLAUDE.md symlinked to AGENTS.md'
      else
        issues << "CLAUDE.md symlink does not resolve to AGENTS.md (target: #{claude_md.readlink})"
      end
    elsif claude_md.file?
      if Digest::SHA256.hexdigest(claude_md.read) == Digest::SHA256.hexdigest(agents_md.read)
        notes << 'CLAUDE.md is a byte-identical copy of AGENTS.md; prefer a symlink'
      else
        issues << 'CLAUDE.md diverges from AGENTS.md (not byte-identical)'
      end
    else
      notes << 'CLAUDE.md missing (expected symlink to AGENTS.md)'
    end
