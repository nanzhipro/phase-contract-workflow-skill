...
* **共享指令**负责让不同 Agent 对项目边界和工作方式保持一致理解，唯一真源是 `AGENTS.md`，`CLAUDE.md` 以符号链接指向它。
...
* **如果你要改仓库级规则，只改 `AGENTS.md`**：它是 Agent 指令的唯一真源，`CLAUDE.md` 是指向它的符号链接，不要复制成实体文件，否则会产生漂移。
...
ruby scripts/planctl doctor                            # 仓库体检（AGENTS.md 存在性与 CLAUDE.md 符号链接检查等）
```
