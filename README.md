...
<p align="center">
  <img src="./assets/phase-contract-logo.svg" alt="Phase-Contract Workflow logo" width="760">
</p>
...
- **机制 > 自觉**。不期望 AI "记得"、"自觉"、"遵守"。进度由脚本原子写回，边界由路径白名单 diff 校验，依赖由 `--strict` 阻断跳步，压缩恢复由 `handoff.md` 充当协议——所有稳定性都来自外部强制，而不是提示词恳求。
...
```bash
# 安装到当前 agent 的默认 skills 目录（自动识别 Copilot / Claude Code / Codex）
