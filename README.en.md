...
* **Shared instructions** keep expectations and guardrails aligned across Agents. `AGENTS.md` is the single source of truth, and `CLAUDE.md` is a symlink to it.
...
* **If you change repository-level rules, edit `AGENTS.md` only**: it is the single source of truth for Agent instructions, and `CLAUDE.md` is a symlink to it — never maintain a separate copy that can drift.
...
ruby scripts/planctl doctor                            # repo health check (AGENTS.md presence, CLAUDE.md symlink check, etc.)
```
