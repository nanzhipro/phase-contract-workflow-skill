...
> 源自真实长任务项目实践的抽象与提炼，剥离业务后可直接套用到任何领域。
...
| P8 | **里程碑外部化** | 每个 phase 完成即 commit + push；有 remote 时形成远端记录，无 remote 时保留本地可回滚里程碑 | AI 先推理哪些新文件属于可再生的中间产物并按需更新 `.gitignore`，然后 `complete` 再自动 `git add -A / commit -F - / push`，地道英文 commit message，失败只 warn 不回滚 state |
...
脚本实现可用任意语言，但必须提供以下 5 条命令。本 Skill 附带的 Ruby 参考实现可直接复用。
