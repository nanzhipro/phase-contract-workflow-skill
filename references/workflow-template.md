...
### Agent 强制层（`AGENTS.md`，`CLAUDE.md` 为符号链接）

`AGENTS.md` 是 agent 指令的唯一真源，被 Codex 及兼容 `AGENTS.md` 协议的通用 agent（含 GitHub Copilot）在会话开头自动注入；`CLAUDE.md` 是指向它的符号链接，供 Claude Code 注入。它作为仓库级 AI 工作流约束，保证 AI 在 phase 相关任务里：
...
- Health 检查（manifest 引用、state/handoff 一致性、AGENTS.md 存在性与 CLAUDE.md 符号链接）
