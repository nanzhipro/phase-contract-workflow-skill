...
| 目标遗忘 / Goal Amnesia | 忘记最初的版本、依赖禁区、质量底线 | `common.md` 永恒硬约束 + `AGENTS.md` 指令强制注入 |
...
Agent 指令在每个会话开场被自动注入，把规约从"建议"变成"前置条件"。`AGENTS.md` 是唯一真源，`CLAUDE.md` 是指向它的符号链接：

| 文件 | 目标 agent |
| --- | --- |
| `AGENTS.md` | Codex / 通用 `AGENTS.md` 协议的 agent（含 GitHub Copilot） |
| `CLAUDE.md` → `AGENTS.md`（符号链接） | Claude Code |

**修改只动 `AGENTS.md`**。`CLAUDE.md` 必须保持为符号链接；任何"仅 Claude 看"的实体副本差异都会让 `planctl doctor` 报错，也会在切换 agent 时产生隐性漂移。差异化诉求改写进 `plan/common.md` 的"按 agent 区分"小节。
...
| `planctl doctor` | 仓库体检：检查 `AGENTS.md` 存在性与 `CLAUDE.md` 符号链接、校验 manifest/state/handoff 引用一致性 |
